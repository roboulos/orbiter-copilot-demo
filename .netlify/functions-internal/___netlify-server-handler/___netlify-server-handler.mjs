...
} from '/var/task/projects/orbiter-copilot-demo/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/projects/orbiter-copilot-demo/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/projects/orbiter-copilot-demo')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/projects/orbiter-copilot-demo',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/projects/orbiter-copilot-demo/.netlify/dist/run/handlers/server.js')
